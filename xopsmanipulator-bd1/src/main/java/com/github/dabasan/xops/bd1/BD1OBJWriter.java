...
class BD1OBJWriter {
	...
	public BD1OBJWriter(Map<Integer, String> texture_filenames_map,
			List<BD1Block> blocks) {
		...
		for (final BD1Block block : blocks) {
			...
			for (int i = 0; i < 6; i++) {
				...
				v1 = VectorFunctions.VSub(vertex_positions[vertex_indices[3]],
						vertex_positions[vertex_indices[0]]);
				v2 = VectorFunctions.VSub(vertex_positions[vertex_indices[1]],
						vertex_positions[vertex_indices[0]]);
			...
			for (int i = 0; i < 6; i++) {
				...
				for (int j = 0; j < 4; j++) {
					faces[i].SetVertexPosition(j,
							vertex_positions[vertex_indices[j]]);
	...
	public int Write(String obj_filename) {
		...
		String mtllib_str;
		mtllib_str = FilenameFunctions
				.GetFilenameWithoutDirectory(obj_filename);
		mtllib_str = FilenameFunctions.GetFilenameWithoutExtension(mtllib_str)
				+ ".mtl";
		String mtl_filename;
		mtl_filename = FilenameFunctions
		...
		for (final Map.Entry<Integer, List<BD1Face>> entry : faces_map
			...
			material_name = FilenameFunctions
					.GetFilenameWithoutDirectory(texture_filename);
			material_name = FilenameFunctions
			...
			for (final BD1Face face : faces) {
				...
				for (int i = 3; i >= 0; i--) {
					obj.addVertex(vertex_positions[i].GetX(),
							vertex_positions[i].GetY(),
							vertex_positions[i].GetZ());
