...
class BD1Writer {
	...
	public int Write(String bd1_filename) {
		...
		for (int i = 0; i < blocks.size(); i++) {
			...
			// Vertex positions
			for (int j = 0; j < 8; j++) {
				ByteFunctions.AddFloatValueToBin_LE(bin,
						vertex_positions[j].GetX());
			}
			for (int j = 0; j < 8; j++) {
				ByteFunctions.AddFloatValueToBin_LE(bin,
						vertex_positions[j].GetY());
			}
			for (int j = 0; j < 8; j++) {
				ByteFunctions.AddFloatValueToBin_LE(bin,
						vertex_positions[j].GetZ());
			}
	...
	private void AddTextureFilenamesToBin(List<Byte> bin) {
		...
		for (int texture_id : sorted_texture_filenames_map.keySet()) {
			...
			for (pos = 0; pos < texture_filename.length(); pos++) {
				texture_filename_buffer[pos] = (byte) texture_filename
						.charAt(pos);
			}
