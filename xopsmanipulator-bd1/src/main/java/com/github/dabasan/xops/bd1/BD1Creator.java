...
public class BD1Creator {
	 ...
	 * 
	 * @param block_handle
	 *            Handle of the block to be duplicated
	...
	public int CreateCube(Vector center, float edge_length) {
		...
		vertex_positions[1] = VectorFunctions.VGet(x - edge_half_length,
				y + edge_half_length, z - edge_half_length);
		vertex_positions[2] = VectorFunctions.VGet(x + edge_half_length,
				y + edge_half_length, z - edge_half_length);
		vertex_positions[3] = VectorFunctions.VGet(x + edge_half_length,
				y + edge_half_length, z + edge_half_length);
		vertex_positions[4] = VectorFunctions.VGet(x - edge_half_length,
				y - edge_half_length, z + edge_half_length);
		vertex_positions[5] = VectorFunctions.VGet(x - edge_half_length,
				y - edge_half_length, z - edge_half_length);
		vertex_positions[6] = VectorFunctions.VGet(x + edge_half_length,
				y - edge_half_length, z - edge_half_length);
		vertex_positions[7] = VectorFunctions.VGet(x + edge_half_length,
				y - edge_half_length, z + edge_half_length);
		...
		for (int i = 0; i < 24; i++) {
			if (i % 4 == 0) {
				block.SetUV(i, 0.0f, 0.0f);
			} else if (i % 4 == 1) {
				block.SetUV(i, 1.0f, 0.0f);
			} else if (i % 4 == 2) {
				block.SetUV(i, 1.0f, 1.0f);
			} else {
				block.SetUV(i, 0.0f, 1.0f);
			}
		}
	...
	public int CreateBlock(Vector[] vertex_positions) {
		...
		if (vertex_positions.length != 8) {
			logger.warn(
					"Invalid number of vertices in the array. vertex_num={}",
	 ...
	 * @return -1 on error and 0 on success
	 */
	public int SetBlockTextureID(int block_handle, int face_index,
			int texture_id) {
		if (blocks_map.containsKey(block_handle) == false) {
			logger.warn("No such block. block_handle={}", block_handle);
			return -1;
